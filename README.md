...
An AI assistant split into two cooperating pieces:

| Component | What it is |
|-----------|-----------|
| **MCP Server** (`uv run buddy`) | A [FastMCP](https://github.com/jlowin/fastmcp) server that exposes tools (news, web search, system info, …) over SSE. Think of it as the backend — it does the actual work. |
| **Voice Agent** (`uv run buddy_voice`) | A [LiveKit Agents](https://github.com/livekit/agents) voice pipeline that listens to your microphone, reasons with an LLM (Gemini 2.5 Flash by default), and speaks back with OpenAI TTS — all while pulling tools from the MCP server in real time. |
...
## Project structure
buddy-ai-assistant/
├── server.py           # uv run buddy  → starts the MCP server (SSE on :8000)
├── agent_buddy.py      # uv run buddy_voice → starts the LiveKit voice agent
├── pyproject.toml
...
└── resources/      # MCP resources exposed to clients (buddy://info)
...
Starts the LiveKit voice agent in **dev mode** — it joins a LiveKit room and begins listening. Open the [LiveKit Agents Playground](https://agents-playground.livekit.io) and connect to your room to talk to Buddy.

---

## `uv run buddy` vs `uv run buddy_voice`

| Command | Entry point | What it does |
|---------|------------|--------------|
| `uv run buddy` | `server.py → main()` | Launches the **FastMCP server** over SSE transport on port 8000. This is the "brain backend" — it registers all tools, prompts, and resources that the LLM can call. |
| `uv run buddy_voice` | `agent_buddy.py → dev()` | Launches the **LiveKit voice agent**. It builds the STT / LLM / TTS pipeline, connects to your LiveKit room, and wires up the MCP server as a tool source. The `dev()` wrapper auto-injects the `dev` CLI flag so you don't have to type it manually. |
...
Open `agent_buddy.py` and change the provider constants at the top:
...
1. Create or open a file in `buddy/tools/`
2. Define a `register(mcp)` function and decorate tools with `@mcp.tool()`
3. Import and call `register(mcp)` inside `buddy/tools/__init__.py`
