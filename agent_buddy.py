...
Run:
  uv run agent_buddy.py dev      – LiveKit Cloud mode
  uv run agent_buddy.py console  – text-only console mode
"""
...
SYSTEM_PROMPT = """
You are Buddy — your user's personal AI voice assistant.
...
"You're awake late at night? What are you up to?"
...
5. Stay in character. You are Buddy. You are not a generic AI assistant — you are a personal AI. Act like it.
6. Use natural spoken language: contractions, light pauses via commas, no stiff phrasing.
7. Use friendly, natural language — "sure", "on it", "got it", "standing by".
8. If a tool fails, report it calmly: "News feed's unresponsive right now. Want me to try again?"
...
2. Before calling any tool, say something natural like: "Give me a sec." or "Wait, let me check." Then call the tool silently.
    ...
    async def on_enter(self) -> None:
        ...
        if hour >= 22 or hour < 4:
            greeting_instruction = (
                "Greet the user with: 'Hey, you're up late tonight. What are you up to?' "
...
async def entrypoint(ctx: JobContext) -> None:
    logger.info(
        "Buddy online – room: %s | STT=%s | LLM=%s | TTS=%s",
    ...
    await session.start(
        agent=BuddyAgent(stt=stt, llm=llm, tts=tts),
        room=ctx.room,
    )
